...
request status:
running - выполняется
suspended - ожидает ресурсы
runable - ожидает CPU

wait - ожидание (некоторых ресурсов: например операции I/O, записи в журнал и пр.)
lock - это "показатель" того, что-данный ресурс используется в работе, each lock is 96 bytes in size, происходит на уровне транзакций (trancaction logical level)
latch - внутренний объект SQL Server, происходит при распараллеливании потока для контроля страниц, происходит на уровни страниц (phisical pages level)
*/



/*все планы запосов в кеше*/
select * from sys.dm_exec_query_stats

/*все планы запросов в кеше*/
select s.* 
, t.text
, p.query_plan
...
/*агрегированная статистика всех ожиданий с начала работы сервера*/
select * from sys.dm_os_wait_stats --where wait_type = 'CXPACKET'

/*агрегированная статистика всех кратковременных блокировок с начала работы сервера*/
select * from sys.dm_os_latch_stats --where wait_type = 'CXPACKET'



/*почистить статистику по блокировкам*/
--DBCC SQLPERF("sys.dm_os_wait_stats", CLEAR)
--DBCC SQLPERF('sys.dm_os_latch_stats', CLEAR);



/*рейтинг кратковременных блокировок*/
WITH Latches AS
	(SELECT
		latch_class,
...
/*стастистика по самым распространенным блокировкам за все время*/
WITH [Waits]
AS (
	SELECT [wait_type]
	, [wait_time_ms] / 1000.0 AS [WaitS]
...
GROUP BY [W1].[RowNum]
HAVING SUM([W2].[Percentage]) - MAX([W1].[Percentage]) < 95;-- percentage threshold GO
